...
namespace DashboarJira.Controller
{
    public class IAIOController
    {
        ...
        const string JQL_GENERAL = "created >= {0} AND created <= {1} AND issuetype = 'Solicitud de Mantenimiento' AND status = Cerrado AND 'Clase de fallo' in AIO AND 'Tipo de componente' = Puerta ORDER BY key DESC, 'Time to resolution' ASC";
        const string JQL_CONTRATISTA = "created >= {0} AND created <= {1} AND issuetype = 'Solicitud de Mantenimiento' AND 'Tipo de causa' = 'A cargo del contratista' AND status = Cerrado AND 'Clase de fallo' in AIO AND 'Tipo de componente' = Puerta ORDER BY key DESC, 'Time to resolution' ASC";
